...
computeTranformation <- function(points_1, points_2)
{
...
X <- matrix(c(x2-x, x-x1),2,1)
Y <- matrix(c(y2-y, y-y1),1,2)
...
tranformImage <- function(n_col, n_row, img_1, img_2, points_1, points_2)
{
  ...
  for(i in 1:n_col)
    for(j in 1:n_ren)
    {
      ...
      img_3[i,j] <- extrapolate(img_1, n_col, n_row, min_val, Tp)
...
writeImage(img1_cropped_neg, "img1.tiff", compression = "none")
writeImage(img2_cropped_neg, "img2.tiff", compression = "none")
