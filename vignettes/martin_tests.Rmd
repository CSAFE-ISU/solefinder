...
avg_ren <- sum((img1_cropped_neg > 0) * (1:n_ren))/(sum(img1_cropped_neg > 0))
avg_col <- sum((t(img1_cropped_neg) > 0) * (1:n_col))/(sum(img1_cropped_neg > 0))

# and then we compute the corresponding slope

y <- array( (img1_cropped_neg > 0.8) * (1:n_ren))
y <- y[y > 0]

x <- array(t(t(img1_cropped_neg > 0.8) * (1:n_col)))
x <- x[x > 0]
